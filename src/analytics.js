...
const criteria = {
	...
	hasTrackingInfo: function(e) {
		return (
			criteria.isElement(e) && e.hasAttribute('data-analytics-category') && e.hasAttribute('data-analytics-action')
		);
	},
...
const delegate = function(filter, listener) {
	return function(evt) {
		let el = evt.target;
		do {
			if (filter && !filter(el)) {
...
export class Analytics {
	...
	_trackClick(evt) {
		...
		const element = this._unwrap(evt.delegateTarget);
		...
		this._log(
			'debug',
			`click: category '${tracking.category}', action '${tracking.action}', label '${tracking.label}', value '${
				tracking.value
			}'`
		);
